...
def cmd_reset(args: argparse.Namespace) -> int:
    ...
    reload the cached engine singleton in-process. ``MAGI_STATE_DIR``
    is set to the parent of the primary DB so the env path
    resolution lands on the right file. ``PATH`` is inherited
    ...
    # ``MAGI_STATE_DIR`` at the directory of the first DB so
    # the env-driven state-dir resolution lands on the right
    # ``magi.db`` (the project's alembic env.py reads it).
    state_dir = db_paths[0].parent
    print(f"\n[reset] alembic upgrade head (state_dir={state_dir})")
    env = os.environ.copy()
    env["MAGI_STATE_DIR"] = str(state_dir)
