...
| Message | `message_id` | — |
| Job (any Job Board) | `job_id` | Includes agent turns: `ChatNotifyJob.job_id` is the `chat_notify_jobs` natural key. |
...
| `event_id` | `job_id` | Alembic initial schema (`chat_notify_jobs`); historical chain predates the 2026.08 collapse |
