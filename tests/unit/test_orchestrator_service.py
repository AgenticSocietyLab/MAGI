...
def test_kubernetes_magis_share_one_postgres_service_but_use_distinct_databases(monkeypatch):
    ...
    backend._apply = MethodType(lambda _self, path, manifest: applied.append((path, manifest)), backend)
    ...
    first_job = next(manifest for path, manifest in applied if path.endswith("/jobs/magi-magis-42-research-db-init"))
    assert "CREATE DATABASE magis_42" in first_job["spec"]["template"]["spec"]["containers"][0]["command"][2]
