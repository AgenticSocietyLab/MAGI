...
from magi.bus.firmwares.jobs.runTaskJob import (
    RunTaskJob,
    runTaskJobBoard,
)
...
def test_run_task_no_double_claim_across_threads(tmp_path: Path, thread_count: int) -> None:
    ...
    def worker() -> None:
        ...
        while True:
            ...
            # success.
            from magi.bus.firmwares.jobs.runTaskJob import RunTaskResult
...
def test_expired_lease_is_reclaimed_exactly_once(tmp_path: Path) -> None:
    ...
    *one* of N concurrent consumers, not N copies of itself.
    ...
    from magi.bus.firmwares.jobs.runTaskJob import _RunTaskJobRow
