...
``admin`` operators can create / delete them. The tools
won't appear in the LLM menu for non-admins.

The LLM is expected to interact with the operator to
collect the right fields before calling ``add_mcp_server``
— some fields are required (name, connection_type), others
are conditional (command+args for stdio, url for sse/http).
