...
Five surfaces pinned:
  ...
  - Contact role not in ``allowed_roles`` → refuse
    with the role repr (``role 'contact'``) so
    callers can keep grepping the error.
  - Happy path (role ∈ allowed_roles) → return ``None``.

Mirrors the same ``fresh_db`` / seed_contacts pattern
used by ``test_action_item_tools.py`` — we need a real
ORM context so the helper's Contact lookup resolves.
"""
...
def test_returns_none_for_permitted_role(fresh_db, seed_contacts):
...
def test_rejects_nonexistent_contact(fresh_db):
    """Contact id that doesn't resolve to a row returns
...
def test_permits_each_role_independently(fresh_db, seed_contacts):
