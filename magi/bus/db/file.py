...
:class:`~magi.bus.db.engine.EngineFactory`: EngineFactory wraps a
SQLAlchemy ``Engine``, FileShelf wraps a local directory.  Both serve
as backends for *Books* (see :mod:`magi.bus.library.file`).
...
class FileShelf:
    ...
    Parallel to :class:`~magi.bus.db.engine.EngineFactory` for
