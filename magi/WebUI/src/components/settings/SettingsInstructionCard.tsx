...
export function SettingsInstructionCard() {
  ...
  useEffect(() => { void apiFetch<{ instruction: string }>("/api/magic/self/instruction").then((x) => { setValue(x.instruction); setSaved(x.instruction); }).catch((e: Error) => setError(e.message)); }, []);
  const save = async () => { setSaving(true); setError(null); try { const data = await apiFetch<{ instruction: string }>("/api/magic/self/instruction", { method: "PUT", body: { instruction: value } }); setValue(data.instruction); setSaved(data.instruction); } catch (e) { setError((e as Error).message); } finally { setSaving(false); } };
