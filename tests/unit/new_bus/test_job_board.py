...
def test_publish_claim_complete(bus: Bus) -> None:
    ...
    bus.submit_result(claimed, BaseJobResult(id=claimed.id))
...
def test_claim_then_fail(bus: Bus) -> None:
    ...
    claimed = bus.claim(PingJob)
    assert claimed is not None
    bus.submit_result(claimed, BaseJobResult(status=JobStatus.FAILED, error="nope"))
    outcome = bus.get_result(claimed)
...
def test_list_filters_status(bus: Bus) -> None:
    first_id = bus.publish(PingJob(n=1))
