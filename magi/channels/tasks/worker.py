...
class TaskWorker(RuntimeWorker):
    ...
    async def _fire_task(self, task, *, fired_by: str = "cron_tick", conversation_id: str | None = None, contact_id: int | None = None) -> None:
        from magi.bus.guild.chatJob import publish_chat
        task_id = task.id; effective_contact_id = contact_id or task.contact_id; effective_conversation = conversation_id or task.conversation_id
        ...
        if effective_conversation and effective_contact_id:
            try:
                await self.call(self.bus.messages_book.add, conversation_id=effective_conversation,
                    role="user", text=contextual_prompt)
            except Exception: pass
        await self.call(
            publish_chat, self.bus, text=contextual_prompt, channel="task",
            contact_id=effective_contact_id, conversation_id=effective_conversation,
    ...
    async def _handle_run_task_job(self, rj: RunTaskJob) -> None:
        ...
        try:
            ...
            await self._fire_task(task, fired_by=rj.fired_by, conversation_id=rj.conversation_id or task.conversation_id, contact_id=rj.contact_id or task.contact_id)
