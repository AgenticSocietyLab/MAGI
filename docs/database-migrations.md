...
- `magis`（一群 MAGI 组成的 Agentic Society = MAGIS 组织树，自引用 `parent_id`。每个 MAGIC Citizen 跑在自己的容器里，MAGIS 不是容器，是组织树节点）
- `magic`（MAGIC 运行时 agent 行 = "MAGI Citizens"，以 `magis_id` 归属 Society，`magic_position` ∈ {`adam`, `eve`}）
